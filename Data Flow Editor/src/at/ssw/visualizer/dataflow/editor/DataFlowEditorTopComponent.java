    ...
    private Toolbar createToolbar() {
        ...
        BUTExportGraph.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                fc.addChoosableFileFilter(new SimpleFileFilter("Graphics Interchange Format (*.gif)", ".gif"));
                fc.addChoosableFileFilter(new SimpleFileFilter("Scaleable Vector Format (*.svg)", ".svg"));
                fc.addChoosableFileFilter(new SimpleFileFilter("Extended Meta File (*.emf)", ".emf"));
                ...
                if (fc.getSelectedFile() != null) {
                    ...
                    if (fileName.endsWith("emf")) {
                        EMFGraphics2D graphics;
                        try {
                            File f = new File(fileName);
                            graphics = new EMFGraphics2D(f, scenePane);
                            graphics.startExport();
                            scenePane.print(graphics);
                            graphics.endExport();
                        } catch (Exception ex) {
                        }
                    }
                    ...
                    if (fileName.endsWith("gif")) {
                        GIFGraphics2D graphics;
                        try {
                            File f = new File(fileName);
                            graphics = new GIFGraphics2D(f, scenePane);
                            graphics.startExport();
                            scenePane.print(graphics);
                            graphics.endExport();
                        } catch (Exception ex) {
                    ...
                    if (fileName.endsWith("svg")) {
                        ...
                        try {
                            File f = new File(fileName);
                            graphics = new SVGGraphics2D(f, scenePane);
                            graphics.startExport();
                            scenePane.print(graphics);
                            graphics.endExport();
                        } catch (Exception ex) {
